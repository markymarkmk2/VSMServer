...
import de.dimm.vsm.lifecycle.RetentionResult;
...
public class Deduptest {
     ...
     */
    @Test
    public void testDedupFileElem() throws SQLException, Throwable
    {
        ...
        String ba_path = "z:\\unittest\\dedupdata";
        File ba_dir = new File(ba_path);
        ba_dir.mkdirs();

        String restore_path = "z:\\unittest\\dedupdatarestore";
        File ra_dir = new File(restore_path);
        ra_dir.mkdirs();
        String restore_path_data = "z:\\unittest\\dedupdatarestore\\dedupdata";
...
//        main.init();
        ...
        // SWITCH TO FULL DEDUP
        pool_handler.getPool().setLandingZone(false);
        ...
        try
        {
            ...
            long ddbl = context.getStatCounter().getDedupBlocks();

            // ONE REAL BLOCK, 9 ARE DEDUPED
            assertTrue(ddbl >=  9);
            ...
            s0 = createSnapshot();
get_fs_handle_for_dedupblock(hb_orig.get(0));
...
//            // NOW HANDLE RETENTION FOR S1, THAT MEANS WE LOOSE LAST UPDATE AND WE LOOSE ORIGINAL BLOCK OF UPDATE
            ...
            // ONE FILE, TWO BLOCKS
            assertEquals(localret.getFiles(), 1 );
            assertEquals(localret.getSize(), 2*bs );
 ...
 int filterRetentionResultList( RetentionResultList retentionResult, long fidx )
    {
        int cnt = 0;
        List<Object[]> l = retentionResult.getList();
