...
public class LogicControl {
    ...
    public RenderEngineManager getRenderEngineManager() {
...
//        System.setProperty("javax.net.ssl.keyStorePassword", getKeyPwd());        
    ...
    void init( boolean agent_tcp ) throws SQLException {
        ...
        // DB prüfen falls neue DB-Version
        if (Main.getRebuildDB()) {
            check_db_changes();
        }

        // RenderFactory initialisieren
        RenderFactory.init();

        // Anpassungen an DB ausführen
        checkFixes();
        ...
        if (licChecker == null) {
            licChecker = new LicenseChecker(PRODUCT_BASE, /*TB*/ 10, /*mods*/ LTM_FTP | LTM_S3 | LTM_PREVIEW);
        ...
        renderEngineManager = new RenderEngineManager();
        workerList.add(renderEngineManager);
    ...
    public StoragePoolHandlerContextManager getPoolContextManager() {
    ...
    final void initNetServer( SearchContextManager scm, StoragePoolHandlerContextManager spcm, boolean agent_tcp ) {
        ...
        try {
            ...
            ServletHolder itunesServlet = VaadinWysiwyg.createGuiServlet("de.dimm.vsm.VaadinTunes");
            netServer.addServletHolder("itunes/*", itunesServlet);

            ServletHolder leuchttischServlet = VaadinWysiwyg.createGuiServlet("de.dimm.vsm.LeuchtTisch");
            netServer.addServletHolder("lts/*", leuchttischServlet);
...
/*        if (Main.isOwnDavServer()) {
            try {
                ...
                FilterHolder fh = new FilterHolder(new VSMWebDavFilter());
                context.addFilter(fh, "/*", null);
                webDavServer.start_server(Main.getWebDavPort(), false, getKeyStore(), getKeyPwd());
            }
            catch (Exception exception) {
                Log.err( "Starten des DAVservers schlug fehl", exception );
            }
        }   
        * */
    }
