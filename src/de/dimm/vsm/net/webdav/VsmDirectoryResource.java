...
public class VsmDirectoryResource  extends VsmResource implements MakeCollectionableResource, PutableResource, CopyableResource, DeletableResource, MoveableResource, PropFindableResource, LockingCollectionResource, GetableResource {
    ...
    public VsmDirectoryResource(String host, VsmDavResourceFactory factory, FileSystemElemNode dir, IVsmContentService contentService) {
        super(host, factory, dir);
        this.contentService = contentService;
        
        if (!dir.isDirectory()) {
            throw new IllegalArgumentException("Is not a directory: " + dir.getAttributes().getName());
        }
    }
...
//        return new VsmDirectoryResource(host, factory, fnew, contentService);
    ...
    public Resource child(String name) {        
        FileSystemElemNode node = factory.getChild( this, name);
    ...
    public List<? extends Resource> getChildren() {
        ...
        List<FileSystemElemNode> files = factory.getChildren(file);
        if (files != null) {
            for (FileSystemElemNode fchild : files) {
                VsmResource res = factory.resolveFile(this.host, fchild);
                if (res != null) {
                    list.add(res);
                } else {
                    log.error("Couldnt resolve file {}", fchild.getAttributes().getName());
                }
            }
        }
        return list;
    ...
    public Resource createNew(String name, InputStream in, Long length, String contentType) throws IOException {
        return null;
                //throw new IllegalArgumentException("Create Dir not supported: " + name);
...
//    }
    ...
    public void sendContent(OutputStream out, Range range, Map<String, String> params, String contentType) throws IOException, NotAuthorizedException {
        String subpath = factory.getSubPath( getFile());
        String uri = subpath;
